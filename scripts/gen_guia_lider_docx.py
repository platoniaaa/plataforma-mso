...
add_para(
    ...
    'el administrador carga los datos del programa. Si todavía no lo recibes, revisa '
    'tu carpeta de spam o contacta al administrador del programa.'
)
...
add_step(2, 'Ingresa tu email corporativo en el campo correspondiente.')
add_step(3, 'Ingresa tu contraseña. Si es tu primer acceso, usa la contraseña que recibiste por correo.')
add_step(4, 'Haz click en el botón "Ingresar".')
...
add_step(2, 'Ingresa tu email corporativo y haz click en "Enviar instrucciones".')
...
add_callout('warning',
    ...
    'de seguridad. Si no lo usas a tiempo, solicita otro.')
...
add_step(3, 'En una encuesta pendiente, haz click en "Responder".')
add_step(4, 'Lee las instrucciones y responde cada pregunta. La mayoría son de tipo escala de niveles (1 a 4).')
...
add_step(5, 'Una barra de progreso te indica cuántas preguntas llevas respondidas.')
add_step(6, 'Al terminar, haz click en "Enviar Respuestas". Recibirás un correo de confirmación.')
...
add_callout('success',
    ...
    'Encuestas Completadas y haz click en "Rehacer evaluación". Tus respuestas anteriores '
...
add_para(
    'Durante el programa responderás cuatro tipos de evaluaciones distribuidas en '
...
add_para(
    'Como líder, tienes un colaborador asignado que participará contigo durante el '
...
add_step(1, 'En el menú lateral, haz click en "Mi Equipo".')
add_step(2, 'Verás una tarjeta con el nombre y cargo de tu colaborador, junto con un contador de feedback registrado.')

add_callout('warning',
    'Si no ves ningún colaborador asignado, contacta al administrador del programa.')
...
add_step(2, 'En la tarjeta de tu colaborador, haz click en "Dar Feedback".')
add_step(3, 'Completa los tres campos obligatorios: Fortaleza observada, Aspecto a reforzar y Recomendación.')
add_step(4, 'Haz click en "Enviar Feedback". Podrás ver el histórico de feedback entregado en la misma vista.')

add_heading('Recomendaciones para dar un buen feedback', level=2, color=MORADO_CLARO)
add_bullet('Sé específico: menciona una situación concreta, no generalidades.')
add_bullet('Enfócate en el comportamiento observado, no en la persona.')
add_bullet('Ofrece recomendaciones accionables.')
add_bullet('Mantén un tono constructivo y respetuoso.')
...
add_step(2, 'Verás una lista de todos los archivos disponibles. Haz click en cada uno para descargarlo o visualizarlo.')
...
add_callout('tip',
    'Estos informes son generados por el administrador. Si quieres acceder al tuyo, '
...
add_para(
    'Si encuentras algún error técnico, tienes una duda o quieres sugerir una mejora, '
    'puedes reportarlo directamente desde la plataforma.'
)
add_step(1, 'En el menú lateral, haz click en "Incidencias".')
add_step(2, 'Completa los campos: categoría, título, descripción del problema y, opcionalmente, adjunta una captura de pantalla.')
add_step(3, 'Haz click en "Reportar". El equipo de soporte recibirá tu reporte y te contactará.')
...
faqs = [
    ('No me llegó el correo de bienvenida, ¿qué hago?',
     'Revisa tu carpeta de spam o correo no deseado. Si tampoco está ahí, contacta al administrador del programa para que verifique el envío.'),
    ('El link de reset de contraseña dice que expiró',
     'Los links de reset son válidos por 1 hora y solo se pueden usar una vez. Si expiró, solicita otro desde "¿Olvidaste tu contraseña?" del login.'),
    ('No veo mi evaluación disponible',
     'Las evaluaciones se activan según el cronograma del programa. Si la fecha ya pasó y no la ves, contacta al administrador para que verifique el estado de activación.'),
    ('Respondí una evaluación por error, ¿puedo corregirla?',
     'Sí. Ve a Mis Evaluaciones → Encuestas Completadas y haz click en "Rehacer evaluación". Tus respuestas anteriores serán reemplazadas.'),
    ('No veo a mi colaborador asignado',
     'Contacta al administrador del programa. Él puede verificar la asignación y corregirla si es necesario.'),
...
add_para('Si necesitas ayuda o tienes alguna duda que no aparece en esta guía, contacta a:')
add_table_header(
    ['Canal', 'Detalle'],
    [
        ...
        ['Módulo de Incidencias', 'Reporta dentro de la plataforma cualquier error técnico.'],
    ]
)
