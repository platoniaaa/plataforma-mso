-- ============================================
-- ESQUEMA DE BASE DE DATOS - PLATAFORMA MSO
...
-- 3. USUARIOS (perfiles vinculados a auth.users)
...
create table archivos_programa (
  id uuid default gen_random_uuid() primary key,
  programa_id uuid not null references programas(id) on delete cascade,
...
-- ============================================
create index idx_programas_cliente on programas(cliente_id);
create index idx_participantes_programa on participantes_programa(programa_id);
create index idx_participantes_usuario on participantes_programa(usuario_id);
create index idx_competencias_programa on competencias(programa_id);
create index idx_encuestas_programa on encuestas(programa_id);
...
create index idx_respuestas_evaluador on respuestas(evaluador_id);
create index idx_archivos_programa on archivos_programa(programa_id);
...
-- Politicas temporales: permitir todo para usuarios autenticados
-- (se refinaran por rol despues)
create policy "Acceso autenticado" on clientes for all using (auth.role() = 'authenticated');
create policy "Acceso autenticado" on programas for all using (auth.role() = 'authenticated');
create policy "Acceso autenticado" on usuarios for all using (auth.role() = 'authenticated');
create policy "Acceso autenticado" on participantes_programa for all using (auth.role() = 'authenticated');
create policy "Acceso autenticado" on competencias for all using (auth.role() = 'authenticated');
create policy "Acceso autenticado" on encuestas for all using (auth.role() = 'authenticated');
create policy "Acceso autenticado" on preguntas for all using (auth.role() = 'authenticated');
create policy "Acceso autenticado" on respuestas for all using (auth.role() = 'authenticated');
create policy "Acceso autenticado" on archivos_programa for all using (auth.role() = 'authenticated');
create policy "Acceso autenticado" on notificaciones for all using (auth.role() = 'authenticated');

-- ============================================
-- USUARIO ADMIN INICIAL
-- ============================================
insert into usuarios (nombre, email, rol, cargo, estado)
values ('Admin Demo', 'admin@mso.cl', 'admin', 'Administrador', 'Activo');
